...
jQuery(document).ready(function ($) {
    ...
    $(".top-panel .tweets").tweet({
        ...
        auto_join_text_url: "",
        loading_text: "Jalan Wonosari KM 11, Srimulyo, Bantul. Telp : 0274 4353 178"
    });
    ...
    if ($('#kids-slider.nivoSlider').length) {
        ...
        $nivoControlGroup.hover(function () {
            fadeInControl($nivoControlGroup)
        }, function () {
            fadeOutControl($nivoControlGroup)
        });
    }
    ...
    if ($('#kids-slider.lofSliderNews').length) {
        ...
        $lofControlGroup.hover(function () {
            fadeInControl($lofControlGroup)
        }, function () {
    ...
    if ($('.widget_categories ul').length) {
        $('.widget_categories ul').accordion({autoHeight: false, header: ".opener", collapsible: true, event: "click", active: false}); // active:false -> collapsed default
    }
    ...
    /*(function () {
        ...
        if ($('#contactform').length) {
            var $form = $('#contactform'),
                    $loader = "<img src='http://localhost/khalifahweb/development/khalifahweb.1.1.1/resource/doc/images/preloader.gif' alt='Loading...' />";
            ...
            $form.submit(function (e) {
                ...
                //var url = "{$config->site_url('public/contact/send_email')}";
                //alert(url);
                //send data to server
                $.post(url+"/send_email", data, function (response) {
    ...
    $('#send_email').click(function (e){
        $.ajax({
            type: "POST",
            url: "http://localhost/khalifahweb/development/khalifahweb.1.1.1/public/contact/send_email",
